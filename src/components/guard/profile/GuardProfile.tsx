...
import { Shield, Clock, MapPin, LogOut, ChevronRight, CheckCircle2, Home, Building2 } from 'lucide-react';
import '../guard.css';
...
export const GuardProfile: React.FC = () => {
  ...
  return (
    <div>
      ...
      {/* Quick Role Switch Actions */}
      <div className="section-heading-row">
        <h3 className="section-title">Role Switcher & Exit</h3>
      </div>
      ...
      <div className="onboarding-features-list" style={{ marginBottom: '1.5rem' }}>
        ...
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
            <Home size={16} style={{ color: '#2563eb' }} />
            <span style={{ fontWeight: 700, color: '#1e3a8a' }}>Switch to Resident App (Sarvesh)</span>
          </div>
        ...
        <button
          className="onboarding-feature-item"
          style={{ width: '100%', cursor: 'pointer', justifyContent: 'space-between', background: '#f8fafc', borderColor: '#cbd5e1' }}
          onClick={() => switchRole('secretary')}
        >
          <div style={{ display: 'flex', alignItems: 'center', gap: '0.625rem' }}>
            <Building2 size={16} style={{ color: '#334155' }} />
            <span style={{ fontWeight: 700, color: '#0f172a' }}>Switch to Secretary Portal (Mayuri)</span>
          </div>
